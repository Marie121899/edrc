          ...
          <div class="card card-info card-outline">
            <div class="card-body box-profile">
              <div class="text-center">
                ...
                src="{{ asset('storage/profiles/' . $organization->profile) }}"
                     alt="User profile picture">
              </div>

              <h3 class="profile-username text-center">{{ $user->name }} / {{ $organization->orgname }}</h3>

              <p class="text-muted text-center">{{ $organization->orgnumber }}</p>
              <p class="text-muted text-center">{{ $organization->dateofregistration }}</p>
            ...
            <div class="card-body">
              <div class="tab-content">
                ...
                <div class="tab-pane fade {{ $redirectToTab === 'timeline' ? 'active' : '' }}" id="timeline">
                  <!-- The timeline -->
                    <form class="form-horizontal" method="POST" action="{{ route('organization.profile.profile') }}">
                      @csrf
                      @method('PUT')
                ...
                <div class="tab-pane fade {{ $redirectToTab === 'settings' ? 'active' : '' }}" id="settings">
                    <form class="form-horizontal" method="POST" action="{{ route('organization.profile.profile') }}">
                        ...
                        @method('PUT')
                      <div class="form-group row">
                        <label for="businessname" class="col-sm-2 col-form-label">Organization Name</label>
                        <div class="col-sm-10">
                          <input type="text" name="businessname" id="businessname" class="form-control" value="{{ $organization->orgname }}">
                        </div>
                      </div>
                      <div class="form-group row">
